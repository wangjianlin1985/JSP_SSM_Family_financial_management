...
public class TradeController
{
    ...
    public String tradeManage(final ModelMap map, final HttpServletRequest request) {
        final List<Datadic> list = this.datadicService.getDatadicTrade();
        map.addAttribute("trades", (Object)list);
        ...
        final List<Shares> shareslist = this.sharesService.getSharesName();
        map.addAttribute("allShares", (Object)shareslist);
        return "tradeManage";
    }
    
    @RequestMapping({ "/tradelist.do" })
    public String list(@RequestParam(value = "page", required = false) final String page, @RequestParam(value = "rows", required = false) final String rows, final Trade s_trade, final HttpServletResponse response) throws Exception {
        ...
        final List<Trade> tradeList = this.tradeService.findTrade(map);
        final Long total = this.tradeService.getTotalTrade(map);
        final JSONObject result = new JSONObject();
        final JSONArray jsonArray = JSONArray.fromObject((Object)tradeList);
    ...
    public String save(final Trade trade, final HttpServletResponse response) throws Exception {
        int resultTotal = 0;
        final JSONObject result = new JSONObject();
        if (trade.getId() == null) {
            resultTotal = this.tradeService.addTrade(trade);
        }
        else {
            resultTotal = this.tradeService.updateTrade(trade);
        }
        if (resultTotal > 0) {
    ...
    public String delete(@RequestParam("ids") final String ids, final HttpServletResponse response) throws Exception {
        ...
        for (int i = 0; i < idsStr.length; ++i) {
            this.tradeService.deleteTrade(Integer.parseInt(idsStr[i]));
        }
        result.put((Object)"errres", (Object)true);
